...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig, bool configFromFlash)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "Altimeter") == 0)
        _customType = ALTIMETER_DEVICE_1;

    if (_customType == ALTIMETER_DEVICE_1) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(Altimeter))) {
        ...
        ********************************************************************************** */
        ...
        _myAltimeter = new (allocateMemory(sizeof(Altimeter))) Altimeter(_pin1, _pin2);
        _myAltimeter->attach(Parameter1, Parameter2);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == ALTIMETER_DEVICE_1) {
        _myAltimeter->set(messageID, setPoint);
    } 
}